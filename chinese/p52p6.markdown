...
修饰符有一个短的和长的形式. 现在老 `/x` 修饰符是默认的, 即空格会被忽略.
    ...
    :s      :sigspace       Every white space in the regex matches 正则表达式匹配所有空格, significant 
...
这的　`:sigspace` 需要花些时间来解释. 它通过 ru　`<.ws>`　替换了模式中的空格　 (that is it calls the rule `ws` without keeping its result).　你可以覆盖该规则.
...
匹配正则表达式时你并不需要编写 grammars, 传统的 `m/.../` 现在还是可以接着使用的, 并且还多了个新的兄弟使法, 就是使用 `mm/.../`, 这是隐含的是使用了 `:sigspace` 修饰符. 记住, 这意味着空格在正则表达式中由 `<.ws>` 规则代替的.
