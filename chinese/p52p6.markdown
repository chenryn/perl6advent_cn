...
这最后一个例子使用的是叫  _colon pair syntax_ 的语法. 当给使用 `:draw-perimeter` 给其赋布尔值的时候, 你什么都不写默认是 `True`, 当想给一个参数赋值为布尔类型的 `FALSE` 的时候，也不需要写明 `0`，而是用以下格式`:!transparent` 
...
它是托管在 github 上, 你可以从 <http://rakudo.org/how-to-get-rakudo> 来下载和安装.
...
#### STD.pm
Larry Wall 为 Perl 6 实现而使用 Perl 6 的 grammar 的实现. 这还有个名字叫 `gimme5`, 它转换 grammar 到 Perl 5. 它可以解析 Perl6 书面的和我们所知道的有效的所有东西, 包括整个测试套件.
...
这时解析开始发挥作用: 象 `identifier(...)` 这种都会解析成子函数调用. 
...
由于传统的异常处理可能会影响隐式的并行性. 所以我们需要一个全其美的解决方案: 不会立即 kill 掉一切, 并且仍然不会丢失任何信息.
...
学习一门语言只靠规范是不够的, 它必须在生产中使用. 相反, 你们需要知道如何解决具体问题. 常用的处理模式叫 _idioms_ 惯用用法, 可以帮助我们在每次有问题的时候不用重新发明轮子.
...
Perl 6 提供了一个方法 `assuming` 在代码的对象上, 它可以传递给它参数给调用者, 并返回的部分的功能的函数.

    my &f := &substr.assuming('Hello, World');

现在  `f(1, 2)` 相当于 `substr('Hello, World', 1, 2)`.

这个  `assuming` 方法也可以工作在操作符上, 因为操作符只是一个奇怪名字的函数. 你想写一个子函数, 给任何传递给它的数都增加 2 , 你可以这样写.
