...
本文开始是以[一系列博客文章][16]的形式存在，为阅读方便收集整理而成。
...
### [SYNOPSIS][47]
    ...
    学习 Perl 6 (如果你已经了解 Perl 5)
    学会并爱上 Perl 6
    知道为什么

### [DESCRIPTION][48]

Perl 6 目前文档不足. 这并不奇怪, 因为编写 Perl 6 的编译器比写文档( 目前只有一个 specification 规范)这个对于用户来讲更加重要多了.

不幸的是，这意味着会让大家不容易学习 Perl 6, 除非你是对 Perl 6 有比较深入的关注, 也就只能从 specification, IRC 和测试套件中学习.

这个项目, 初步给名字定为 "Perl 5 to 6"(目前我找不到更好的名字) 来填补文档空白这个事情.

这个课程都内容都相当矮小精悍. 并试图用很短的例子来解释 42-3 个重点. 也会解释从 Perl 5 变成 Perl 5 的改步, 这一点非常重要. 我希望你通过读这些课程学到所需要的知识. 并基本明白 Synopses 这个东西是 Perl 智慧的来源.

为了保证读起来方便, 每个课程会控制在不超过 200 行或者 1000 个单词(当然就是个软限制);

这个对于学习编程语言也许太短了. 但我希望是讲出语言的设计, 让你看到它强大美丽的一面, 并且不用按语言大纲学习就会使用.
...
Perl 6 这些部分很象 Perl 5 - 只会更加好. 声明的语句以分号终止. 一个代码块的最后一条语句和在一行结束的右大括号的分号终止符是可选的.
...
数组和哈希只有当后面跟一个索引的时候才会插值(或者方法调用, 象 `"some $obj.method()"`), 空索引会给整个数据结构做插值操作.

当出现了一个花括号时会执行花括号中的代码给结果进行值操作.

#### [Arrays][57]

数组还是以 `@` 标记开始. 并且他们总是这样, 就算访问数组中的元素. ie. 这个索引需要存在, 这个对新手学习就不在会晕了.
...
既然一切都是对象，你可以调用数组的方法:
    ...
    my @b = @a.sort;
    @b.elems;                   # 元素的数量 
    if @b > 2 \{ say "yes" \}     # 老的方法仍能工作 
    @b.end;                     # 最后一个索引的编号, 替换  $#array
    my @c = @b.map(\{$\_ \* 2 \});  # map 这也是方法调用 
...
Perl 6 在列表上下文时哈希是一个列表对. 列表对可以做一些其它的事情, 它会是子函数的名字, 稍后详述.
...
最后, 你需要知道这二个 `[..]` 和 `{...}`(一个词语之后直接加上), 这只是一个方法调用的特殊语法, 不是依赖于数组和哈希. 这意味着, 他们也不会依赖于特定的前缀符号.

    my $a = [1, 2, 3];
    say $a[2];          # 3

这意味着, 我们不需要特殊的解引用的语法, 并且您可以创建能够同时充当数组, 哈希和 sub 子函数的对象.
...
许多小变化使得规则更加容易编写. 例如, dot `.` 在匹配任何字符. 旧的语义是除了换行, 可以使用 `\N` 来实现.

修饰符现在在一个正则表达式的起始, 并且非捕获组是 `[...]`, 这比起旧的 `(?:...)` 更加容易读取和写.
...
所有匹配变量是别名为`$/`，也就是所谓的 _Match object_, 它实际上包含了一个完整的匹配树。
